...
ワークスペース見出し行の **ワークスペース ＞ …** パンくずの直前に、ほかの拡張機能のコントロールと同じオレンジ枠の **Recent Workspaces**（最近のワークスペース）プルダウンを表示します。パンくずは右へ移動するため、左右のペインや曲詳細パネルを覆いません。

- 開くと、Sunoが表示する最近使ったワークスペースの先頭10件から選べます。
- 選択はSuno自身の可視ワークスペース行へ委譲するため、非公開APIの呼び出しやワークスペース情報の保存をせず、改名・アーカイブの状態にも追従します。
...
マスタリングプロンプト、プリセット、テイク履歴、自動設定、曲名フォーマット、テイク番号などの設定はすべてお使いのブラウザ内にのみ保存され、開発者が運営するサーバーへ送信されることはありません。Sunoの非公開APIを呼び出すこともなく、Suno保存スタイルのプロンプト本文を読み取っても、それを恒久的に保存することはありません。設定のエクスポートはお使いの端末上にファイルを保存するだけで、どこにもアップロードされません。詳細は[プライバシーポリシー](PRIVACY.md)をご覧ください。
