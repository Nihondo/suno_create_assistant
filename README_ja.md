...
- スタイル本文に書かれていない項目は、プルダウンに触れただけでは書き込まれません。キーを選ぶ、30〜300 BPMのテンポを入力する、拍子を選ぶといった操作のあと、チェックマークの反映ボタンを押します（値がスタイル本文と異なる間だけ有効になります）。拡張機能が管理する `Musical settings:` 行が追加され、以降はその行内が更新されます。選べる拍子は 2/4、3/4、4/4、5/4、6/8、7/8、9/8、12/8 です。
- プルダウンを「未選択」に戻しても、スタイル本文の語句は削除されません。拡張機能の `Musical settings:` 行からその項目が外れるだけです（3項目すべてを未選択にすると行ごと消えます）。
